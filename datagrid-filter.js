DatagridFilter = function(grid,toolbarFlag){
  ...
  var toolbar=[{
    ...
    handler:function(){
      ...
      opt.filterRow=!opt.filterRow;

      //render table 分页时需要注意
      if(grid.datagrid('getPager').length!=0)
        grid.datagrid('getPager').pagination('select',grid.datagrid('getPager').pagination('options').page);
      else
        grid.datagrid({data:grid.datagrid('getData')});
    }
  }];
  //添加过滤的toolbar
  if(toolbarFlag)
...
$.extend($.fn.datagrid.defaults.view, {
  onAfterRender: function (target) {
    ...
    //add filter row
    if (dc.header2.find('[filter="true"]').length == 0 && opts.filterRow) {
            ...
            if (dgData[0] && dgData[0][field] && typeof dgData[0][field] == 'number' && (value.charAt(0) > '9' || value.charAt(0) < '0')) {
              ...
              if (optype > 0 && reg.test(value) && (value = parseInt(value))) {
                for (var i = 0; i < dgData.length; i++) {
                  DatagridFilter.clearHighlight(dc, i, field);
                  if(idx==0||(idx>0 && dc.body1.find('tr[datagrid-row-index="' + i + '"]').is(':visible'))) {
                    if ((optype == 1 && dgData[i][field] >= value)||(optype == 2 && dgData[i][field] <= value)
                      ||(optype == 3 && dgData[i][field] != value)||(optype == 4 && dgData[i][field] > value)
                      ||(optype == 5 && dgData[i][field] < value)||(optype == 6 && dgData[i][field] == value)) {
                        dc.body1.find('tr[datagrid-row-index="' + i + '"]').show();
                        dc.body2.find('tr[datagrid-row-index="' + i + '"]').show();
                    }
                    else{
    ...
    //remove filter row
    if(dc.header1.find('[filter="true"]').length != 0 && !opts.filterRow){
      dc.header1.find('[filter="true"]').remove();
    }
    if(dc.header2.find('[filter="true"]').length != 0 && !opts.filterRow){
      dc.header2.find('[filter="true"]').remove();
    }
